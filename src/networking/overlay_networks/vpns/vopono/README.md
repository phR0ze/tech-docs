...
- [Provider configuration](#provider-configuration)
  - [Mullvad](#mullvad)
    - [Configure Mullvad](#configure-mullvad)
    - [Launch Mullvad](#launch-mullvad)
...
**Example**
```bash
$ voponon exec --provider mullvad --server us_west --protocol wireguard firefox
$ voponon exec --provider mullvad --server us_west --protocol wireguard chromium
```
...
### Mullvad
Mullvad WireGuard credentials live in separate synced .conf files under `~/.config/vopono/mullvad/`,
generated by the interactive `vopono sync` (which registers a device with Mullvad's API using your
account number)

#### Configure Mullvad
1. Run: `vopono sync Mullvad`
2. For `Handle Mullvad key upload automatically` choose `Y`
3. You'll need to enter your Mullvad account number
4. For `Would you like to generate a new keypair?` choose `Y`
5. Use the default port number

this generates the configuration file `~/.config/vopono/mv/wireguard/wireguard_device.json` that
contains a generated private key and public key. The public key is then published to your Mullvad
account using your account number.

#### Launch Mullvad
```bash
$ vopono exec --interface br0 --provider Mullvad --server usa-usslc301 --protocol wireguard firefox
 ```
...
3. Execution would look like
   ```bash
   $ vopono exec --provider PrivateInternetAccess --server us-saltlakecity --protocol wireguard firefox
   ```
